...
int main(int argc, char **argv[])
{
    ...
    for (int i=0; i<30; i++){
        int test = rand()%15; //Primer disparo del robot
    ...
    if(fd!=-1){
        printf("Eliminando archivo existente: %s\n", cadena);
        fd = remove(cadena);

        if(fd == 0) {
            printf("Buffer eliminado correctamente");
        } else {
            printf("Error: No se ha podido eliminar el buffer");
        }
    }else{
